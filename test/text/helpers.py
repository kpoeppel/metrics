...
from test.helpers.testers import MetricTester, _assert_allclose, _assert_requires_grad, _assert_tensor
...
class TextTester(MetricTester):
    """Class used for efficiently run alot of parametrized test in ddp mode. Makes sure that ddp is only setup once
    and that pool of processes are used for all test.

    All test for text metrics should subclass from this and implement a new method called `test_metric_name` where the
...
def skip_on_connection_issues(reason: str = "Unable to load checkpoints from HuggingFace `transformers`."):
    """Wrapper which handles HF-related test if they fail due to connection issues.

    The test run normally if no connection issue arises, and they're marked as skipped otherwise.
    """
